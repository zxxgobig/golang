...
func InsertPr(ps []profit_item, ps_max profit_item, lastcalcdate uint32) (err error) {
	i := 0
	insert_sql := "INSERT INTO sighting_profit (stockcode,rate,initdate,cleardate) VALUES"
	for _, item := range ps {
		if item.cleardate > lastcalcdate {
			insert_sql += fmt.Sprintf("('%s',%f,%d,%d),", item.stockcode, item.rate, item.initdate, item.cleardate)
			i++
		}
	}
	...
	if i > 0 {
		...
		insert_sql += ";"
		log.Release("[InsertPr insert_sql] sql:[%s]", insert_sql)
		count := 0
		for {
	...
	// 更新股票最大收益率
	if ps_max != (profit_item{}) {
		insert_max_sql := fmt.Sprintf(`INSERT INTO sighting_profit_max (stockcode,rate,initdate,cleardate) VALUES('%s',%f,%d,%d) ON DUPLICATE KEY UPDATE stockcode='%s',rate=%f,initdate=%d,cleardate=%d;`,
			ps_max.stockcode, ps_max.rate, ps_max.initdate, ps_max.cleardate,
			ps_max.stockcode, ps_max.rate, ps_max.initdate, ps_max.cleardate)
		count := 0
		log.Release("[InsertPr insert_max_sql] sql:[%s]", insert_max_sql)
		for {
			_, err = global.GServer.GetMysqlEngine().Exec(insert_max_sql)
			if err != nil {
				count++
				if count >= 20 {
					break
...
func InsertBS(sql_items []sqlBsItem) (err error) {
	i := 0
	insert_sql := "INSERT INTO tb_sighting_v2 (stockcode,tradday,stype,open,close,reason) VALUES"
	for _, item := range sql_items {
		insert_sql += fmt.Sprintf("('%s',%d,%d,%d,%d,'%s'),", item.stockcode, item.tradday, item.stype, item.open, item.close, item.reason)
		i++
	}
	if i > 0 {
		insert_sql = insert_sql[:len(insert_sql)-1]
...
func GetLastBSTypeAndVirtual(stock_code string) (stype int, err error) {
	sql_tt := fmt.Sprintf("SELECT tradday, stype FROM tb_sighting_v2 WHERE stockcode='%s' ORDER BY tradday desc limit 1;", stock_code)
...
func GetLastCloseByStock(stock_code string, stype int) (tradday, close int, err error) {
	sql_tt := fmt.Sprintf("SELECT tradday, stype, close FROM tb_sighting_v2 WHERE stockcode='%s' and stype = %d ORDER BY tradday desc limit 1;", stock_code, stype)
	rows, err := global.GServer.GetMysqlEngine().Query(sql_tt)
	if err != nil {
		return
	}
