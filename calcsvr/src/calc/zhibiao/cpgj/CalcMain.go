...
func (self *BSCalculator) Calc() (error, uint32) {
	log.Release("cpgj-2.0 bs point calculate start ")
	...
	for stock_code, stock_item := range global.StockData {
		...
		//获取stock_item.Cbs[i].Tradday >= 20180101 的索引 cfgday
		//每次都从头开始算,但买卖点是根据上一个交易日来生成的
		cfgday := sort.Search(len_cbs, func(i int) bool { return stock_item.Cbs[i].Tradday >= uint32(etc.Config.Calc.Common_StartDate) })
		if cfgday >= len_cbs-1 {
			continue
		}
		...
		//for i:=1; i<len_rc2; i++ {
		//	//收盘价>5日均线 And 收盘价>10日均线 And 收盘价>20日均线 And 看多条件2成立 And 前1交易日看多条件2不成立 And 个股涨跌幅>3%
		...
		//for i := 1; i < len_rc1; i++ {
		//	//最近10个交易日的最高收盘价
		//	max_close := iCbs[i+61].FQKLine.Close
		//	for k := 1; k < 10; k++ {
		//		if iCbs[i+61-k].FQKLine.Close > max_close {
		//			max_close = iCbs[i+61-k].FQKLine.Close
		//		}
		//	}
		//	//全部取均线数据的最后(len_rc1-1)个值: len_rc1 >= 4
		//	//如：当len_rc1 == n时,全部取均线数据的最后(n-1)个值
		//	//分别表示:	前n-2个交易日的[5,10,20,30,60]日均线
		//	//			前n-3个交易日的[5,10,20,30,60]日均线
		//	//			前n-4个交易日的[5,10,20,30,60]日均线
		//	//			......
		//	// 			前2个交易日的[5,10,20,30,60]日均线
		//	//		   	前1个交易日的[5,10,20,30,60]日均线
		//	//		   	前0个交易日的[5,10,20,30,60]日均线(最后一个交易日)
		//	//随着股票K线条数的增加,n也在同步增加
		//	tmp := iCbs[i+61].FQKLine.Close > ma5[i+57].Value && //收盘价 > 5日均线
		//		iCbs[i+61].FQKLine.Close > ma10[i+52].Value && //收盘价 > 10日均线
		//		iCbs[i+61].FQKLine.Close > ma20[i+42].Value && //收盘价 > 20日均线
		//		iCbs[i+61].FQKLine.Close > ma30[i+32].Value && //收盘价 > 30日均线
		//		iCbs[i+61].FQKLine.Close > ma60[i+2].Value && //收盘价 > 60日均线
		//		iCbs[i+61].FQKLine.Close == max_close && //收盘价 = 最近10日个交易日最高收盘价
		//		(rc1[i] || rc2[i+42-1]) //看多条件=True (65根K线时 len(rc2) == 46-1)
		//	buy3 = append(buy3, tmp)    //翻多: 65根K线时 len(buy3) == 4-1
		//}

		//log.Debug("len_rc1 = %v, len_rc2 = %v, len_buy1 = %v, len_buy2 = %v, len_buy3 = %v ", len_rc1, len_rc2, len(buy1), len(buy2), len(buy3))
		//log.Debug("ma5 = %v, ma10 = %v,ma20 = %v, ma30 = %v,ma60 = %v, len_iCbs = %v", len(ma5), len(ma10), len(ma20), len(ma30), len(ma60), len(iCbs))
		...
		//获取某只股票最后一次的买卖点状态和虚拟仓位
		stype, virtual, err := GetLastBSTypeAndVirtual(stock_code)
		if err != nil {
			log.Error("[GetLastBSTypeByStockCode error] stock_code:%s, err:%s", stock_code, err.Error())
			continue
		}
		...
		for k := 0; k < min_len; k++ { //相当于遍历 buy1，因为buy1长度最小
			...
			case STATE_CLEAR: //初始状态和清仓状态一个操作
				fallthrough
			case STATE_INIT: //建仓
				create := bool(virtual == 0 && (stype == STATE_CLEAR || stype == STATE_INIT))
			...
			case STATE_SETUP: //建仓部位开始: 先判定清仓, 不满足所有清仓条件时再判定加仓
				...
				clear := bool(virtual != 0 && stype != STATE_CLEAR && stype != STATE_INIT)
				plus := bool(virtual == 50 && stype == STATE_SETUP)
				...
				cbs[k].C_BsPoint.State = prev_state
			case STATE_PLUS: //加仓部位开始:  先判定清仓, 不满足所有清仓条件时再判定减仓
				if stock_item.BuyClose <= 0 {
					tradday, lastClose, err := GetLastCloseByStock(stock_code, 1) //获取最后一次建仓点收盘价
					if err != nil && lastClose <= 0 {
						log.Error("[GetLastCloseByStock error] stock_code:%s, err:%s", stock_code, err.Error())
				...
				clear := bool(virtual != 0 && stype != STATE_CLEAR && stype != STATE_INIT)
				minus := bool(virtual == 100 && stype == STATE_PLUS)
				if (!rc1[k+1] && close2) || (rc1[k+1] && close4) {
					reason := "[清仓1]-->有效跌破20日均线"
					virtual = 0
					stype = STATE_CLEAR
				...
				} else if cbs[k].FQKLine.Close/stock_item.AddClose <= 0.96 {
					reason := "[减仓1]-->加仓部位止损"
					virtual = 50
					stype = STATE_MINUS
					prev_state = stype
					if cbs[k].Tradday > self.Last_calc_date && minus {
						log.Release("[减仓1 success] stock_code:%s, reason: %s", stock_code, reason)
						sql_items = append(sql_items, sqlBsItem{
							stockcode: stock_code,
							tradday:   cbs[k].Tradday,
							stype:     stype,
				...
				} else if cbs[k].FQKLine.Close/stock_item.AddClose >= 1.08 && cbs[k].FQKLine.Close < ma5[index_ma5+k].Value {
					reason := "[减仓2]-->加仓部位止盈"
					virtual = 50
					stype = STATE_MINUS
					prev_state = stype
					if cbs[k].Tradday > self.Last_calc_date && minus {
						log.Release("[减仓2 success] stock_code:%s, reason: %s", stock_code, reason)
						sql_items = append(sql_items, sqlBsItem{
							stockcode: stock_code,
							tradday:   cbs[k].Tradday,
							stype:     stype,
				...
				} else if IsMoreThanTenDays(stock_code, stock_item.AddDay, iCbs, k, len(iCbs)-min_len) && stock_item.AddClose == max_close11 {
					reason := "[减仓3]-->加仓部位时间止损"
					virtual = 50
					stype = STATE_MINUS
					prev_state = stype
					if cbs[k].Tradday > self.Last_calc_date && minus {
						log.Release("[减仓3 success] stock_code:%s, reason: %s", stock_code, reason)
						sql_items = append(sql_items, sqlBsItem{
							stockcode: stock_code,
							tradday:   cbs[k].Tradday,
							stype:     stype,
			...
			case STATE_MINUS: //减仓部位开始
				...
				if (!rc1[k+1] && close2) || (rc1[k+1] && close4) {
					...
					prev_state = stype
					if cbs[k].Tradday > self.Last_calc_date && clear {
						log.Release("[减仓后清仓1 success] stock_code:%s, reason: %s", stock_code, reason)
						sql_items = append(sql_items, sqlBsItem{
							stockcode: stock_code,
							tradday:   cbs[k].Tradday,
							stype:     stype,
					...
					prev_state = stype
					if cbs[k].Tradday > self.Last_calc_date && clear {
						log.Release("[减仓后清仓2 success] stock_code:%s, reason: %s", stock_code, reason)
						sql_items = append(sql_items, sqlBsItem{
							stockcode: stock_code,
							tradday:   cbs[k].Tradday,
							stype:     stype,
					...
					prev_state = stype
					if cbs[k].Tradday > self.Last_calc_date && clear {
						log.Release("[减仓后清仓3 success] stock_code:%s, reason: %s", stock_code, reason)
						sql_items = append(sql_items, sqlBsItem{
							stockcode: stock_code,
							tradday:   cbs[k].Tradday,
							stype:     stype,
					...
					prev_state = stype
					if cbs[k].Tradday > self.Last_calc_date && clear {
						log.Release("[减仓后清仓4 success] stock_code:%s, reason: %s", stock_code, reason)
						sql_items = append(sql_items, sqlBsItem{
							stockcode: stock_code,
							tradday:   cbs[k].Tradday,
							stype:     stype,
					...
					prev_state = stype
					if cbs[k].Tradday > self.Last_calc_date && clear {
						log.Release("[减仓后清仓5 success] stock_code:%s, reason: %s", stock_code, reason)
						sql_items = append(sql_items, sqlBsItem{
							stockcode: stock_code,
							tradday:   cbs[k].Tradday,
							stype:     stype,
	...
	if flags > 0 {
		log.Release("cpgj bs point sql operation done, A total of %d stocks produced BS point", flags)
...
func (self *BSCalculator) Start() error {
	//回测结果入库：执行一次即可, 执行前需清空表 cpgjv2_back_test
	//OpMain()
	// 计算最后一次的计算时间。
...
func (self *BSCalculator) UpdateCalcDateToMySql(tablename, filedname string, last_calc_time uint32) error {
	insert_sql := fmt.Sprintf(`INSERT INTO %s(stype,start_date,last_calc_date) VALUES ('%s',%d,%d) ON DUPLICATE KEY UPDATE start_date=%d,last_calc_date='%d';`,
		tablename, filedname,
		etc.Config.Calc.Common_StartDate, last_calc_time,
		etc.Config.Calc.Common_StartDate, last_calc_time)
...
func (self *BSCalculator) GetLastCalcDate(tbname, filedname, calcname string) {
	if tbname == "" || filedname == "" {
		...
		self.Last_calc_date = uint32(etc.Config.Calc.Common_StartDate)
		return
	}
	defer func() {
		log.Release("GetLastCalcDate() --cpgj--Last_calc_date = %v", self.Last_calc_date)
	}()

	sql := fmt.Sprintf("SELECT start_date,last_calc_date from %s WHERE stype='%s';", tbname, filedname)
	...
	if start_date != 0 {
		self.Last_calc_date = start_date
		return
	}
	if etc.Config.Calc.Common_StartDate != 0 {
		self.Last_calc_date = uint32(etc.Config.Calc.Common_StartDate)
		return
	}
